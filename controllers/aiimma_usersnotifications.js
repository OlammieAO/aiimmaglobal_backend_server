...
const model = require("../models/aiimma_usersnotifications");
...
exports.getByID = async (req, res, next) => {
	try {
		...
		const offset = (pageNo - 1) * pageSize;
		const ID = req.params.ID;
		const totalCount = await model.getByIDCount(ID);
		const data = await model.getByID(offset, pageSize, ID);
		if (!_.isEmpty(data)) {
			const result = {
				pageNo: pageNo,
				pageSize: pageSize,
...
exports.getByRolecode = async (req, res, next) => {
	try {
		const pageNo = await getPageNo(req);
		const pageSize = await getPageSize(req);
		const offset = (pageNo - 1) * pageSize;
		const RoleCode = req.params.RoleCode;
		const totalCount = await model.getByRolecodeCount(RoleCode);
		const data = await model.getByRolecode(offset, pageSize, RoleCode);
		if (!_.isEmpty(data)) {
			const result = {
				pageNo: pageNo,
				pageSize: pageSize,
