...
const model = require("../models/aiimma_roles");
...
exports.getById = async (req, res, next) => {
	try {
		...
		const data = await model.findById(RoleCode);
		if (!_.isEmpty(data)) {
...
exports.update = async (req, res, next) => {
	try {
		...
		const data = await model.update(RoleCode, req.body);
		if (!_.isEmpty(data)) {
			res.status(StatusCodes.OK).send(data[0]);
		} else {
...
exports.remove = async (req, res, next) => {
	try {
		...
		//const id = req.params.id;
		const data = await model.remove(RoleCode);
		if (data) {
